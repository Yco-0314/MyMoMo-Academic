...
def _normalize_heritable_strategy(population_dynamics, payoff_games, agent_state_vars) -> None:
    ...
    In an evolutionary game the strategy under selection MUST be INHERITED (∝
    fitness) and MUST NEVER be RESET to its init each generation. Either error
    makes the Moran turnover a no-op on strategy and freezes the population at its
    monomorphic start:
      - re-run #2: strategy missing from inherit_attrs → offspring keep the dead
        agent's own strategy, selection never propagates the fit parent's.
      - re-run #4: strategy placed in reset_attrs → every offspring's strategy is
        reset to its init (e.g. all-Hawk), wiping the evolving distribution each
        generation; the population froze at ~0.99.
    Extraction does both — it describes "offspring inherit strategy" in prose but
    fields it wrong. So when both `population_dynamics` and `payoff_games` are
    present, every `payoff_games[].strategy_var` that names a real agent_state_var
    is MOVED into inherit_attrs and OUT of reset_attrs. A structural necessity, not
    a judgement call: the selected trait is heritable and is never reset.
    """
    ...
    for g in payoff_games:
        ...
        if not sv or sv not in state_var_names:
            continue
        # never reset the selected trait (re-run #4: reset → frozen)
        if sv in population_dynamics.reset_attrs:
            population_dynamics.reset_attrs = [
                a for a in population_dynamics.reset_attrs if a != sv
            ]
        # always inherit it (re-run #2: missing → no selection on strategy)
        if sv not in population_dynamics.inherit_attrs:
