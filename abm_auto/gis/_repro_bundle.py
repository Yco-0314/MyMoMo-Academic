"""Replayable reproduction bundle (the trust-layer distribution primitive).
...
Reusable across reproductions — the real Ba-DEM Anshuka run is the
...
def build_bundle(
    *,
    ...
    Optional journal-grade L3 carriers (all default-off, backward-compatible):
    ...
    stored verbatim; ``benchmark`` is a free-form cross-tool baseline dict
